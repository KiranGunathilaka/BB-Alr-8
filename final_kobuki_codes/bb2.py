...
class ColorSequenceRobot:
    def __init__(self):
        ...
        # ROI parameters (default to full frame)
        self.roi_enabled = False
        self.roi_top = 0
        self.roi_bottom = self.height
        self.roi_left = 0
        self.roi_right = self.width
        ...
        self.goal_area_threshold = 70000  # Adjust based on your camera and objects
    ...
    def set_roi(self, top, bottom, left, right, enabled=True):
        """
        Set the Region of Interest for object detection.
        
        Args:
            top: Top coordinate of ROI (pixels from top of frame)
            bottom: Bottom coordinate of ROI (pixels from top of frame)
            left: Left coordinate of ROI (pixels from left of frame)
            right: Right coordinate of ROI (pixels from left of frame)
            enabled: Whether to enable ROI (default: True)
        """
        # Validate input parameters
        if top < 0 or top >= self.height or bottom <= 0 or bottom > self.height or top >= bottom:
            print("Invalid top/bottom ROI values. Using defaults.")
            top = 0
            bottom = self.height
        
        if left < 0 or left >= self.width or right <= 0 or right > self.width or left >= right:
            print("Invalid left/right ROI values. Using defaults.")
            left = 0
            right = self.width
        
        # Set ROI parameters
        self.roi_top = int(top)
        self.roi_bottom = int(bottom)
        self.roi_left = int(left)
        self.roi_right = int(right)
        self.roi_enabled = enabled
        
        print(f"ROI set to: top={self.roi_top}, bottom={self.roi_bottom}, left={self.roi_left}, right={self.roi_right}, enabled={self.roi_enabled}")
    ...
    def detect_color_object(self, frame, goal_mode=False):
        ...
        if self.roi_enabled:
            ...
            # If ROI is empty, return no detection
            if roi_frame.size == 0:
                print("Warning: ROI has zero size")
        ...
        if best_contour is not None:
            ...
            # Adjust coordinates for ROI if enabled
            if self.roi_enabled:
                x_global = x + self.roi_left
                y_global = y + self.roi_top
            else:
                x_global = x
                y_global = y
            ...
            if self.roi_enabled:
                cv2.putText(
                    display_frame,
                    f"ROI: {self.roi_left},{self.roi_top} to {self.roi_right},{self.roi_bottom}",
                    (10, 90),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.6,
                    (255, 255, 255),
            ...
            # Copy display frame back to original
            frame[:] = display_frame
            
            # Return coordinates relative to the whole frame (not ROI)
            return True, x_global, y_global, w, h, max_area, center_x
        else:
            # No object detected
            cv2.putText(
                display_frame,
            ...
            if self.roi_enabled:
                # For ROI mode, resize mask to fit in corner
                small_mask = cv2.resize(mask, (160, 120))
                display_frame[10:130, self.width - 170 : self.width - 10] = cv2.cvtColor(
    ...
    def is_box_captured(self, y, h):
        ...
        # If ROI is enabled, adjust the capture threshold relative to ROI
        if self.roi_enabled:
            CAPTURE_THRESHOLD = 0.9
            bottom_threshold = self.roi_top + (self.roi_bottom - self.roi_top) * CAPTURE_THRESHOLD
            return y > bottom_threshold
        else:
            CAPTURE_THRESHOLD = 0.9
            return y > self.height * CAPTURE_THRESHOLD
...
def main():
    ...
    # Adjust retreat time if needed (default is 5000 ms)
    robot.retreat_time = 5000

    # Adjust goal area threshold based on your objects and camera
    robot.goal_area_threshold = 70000
    
    # Configure ROI if needed (25% from edges)
    # Format: set_roi(top, bottom, left, right, enabled)
    robot.set_roi(
        top=int(robot.height * 0.15),        # Top 25% 
        bottom=int(robot.height * 0.95),     # Bottom 25%
        left=int(robot.width * 0.05),        # Left 25%
        right=int(robot.width * 0.95),       # Right 25%
        enabled=True                          # Enable ROI
    )
    
    print("Robot configured. Press 'q' to quit, 'f'/'g' to adjust forward time, 'r' to reset, 'i' to toggle ROI.")
